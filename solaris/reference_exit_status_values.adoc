...
Errore di amministrazione
 a| 
Il file della password ha una voce errata. Utilizzare `snapdrive config delete` per eliminare la voce di accesso per questo sistema di storage. Quindi, immettere nuovamente le informazioni di accesso utilizzando `_snapdrive config setuser_name_` comando.
...
Errore di amministrazione
 a| 
Il file della password non contiene alcuna voce per questo sistema di storage. Eseguire `_snapdrive config setusername filername_` Comando per tutti i sistemi storage su cui è necessario eseguire SnapDrive per UNIX. Quindi provare a eseguire nuovamente questa operazione.
...
Se la preparazione automatica dell'host è disattivata, è necessario utilizzare il `snapdrive config prepare luns` Per preparare l'host al provisioning dei LUN o per eseguire manualmente le fasi di preparazione.
